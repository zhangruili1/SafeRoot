全球预设库地址：https://github.com/zhangruili1/SafeRoot-default-library
欢迎大家贡献本预设库不针对任何软件完全由个人玩家自定义
...
#### 智能域名采集
- **新增「智能采集」功能**：输入关键词（如"360"），自动通过多来源发现相关域名并批量添加到屏蔽列表。
  - **crt.sh 证书透明度日志**：查询公开的 SSL 证书记录，发现域名持有者的所有子域名，数据量大、覆盖面广。
  - **子域名字典枚举**：基于 150+ 常见子域名前缀，通过 DNS 解析验证是否真实存在。
  - **本地 hosts 文件搜索**：匹配当前已屏蔽规则中的相关域名。
  - 采集结果以表格展示，支持全选/反选、预览后一键添加。
  - **影响文件**：`domain_collector.py`（新增）、`collect_domain_dialog.py`（新增）、`main_window.py`

#### 预设库自动更新
- **新增 GitHub 预设库**：从远程仓库 `zhangruili1/SafeRoot-default-library` 自动同步预设域名库。
  - **启动自动更新**：每次启动 SafeRoot 后台静默检查并下载最新预设库，通过 ETag / Last-Modified
    判断是否有更新，无更新则跳过，不阻塞 UI、不弹窗干扰。
  - **一键导入**：工具栏新增「预设库导入」按钮，点击后显示预设库域名总数和待导入数量，
    确认后批量添加，自动跳过已存在的域名。
  - **增量机制**：本地缓存预设库元数据（ETag、更新时间），仅在远程有变化时才重新下载。
  - 预设库存储路径：`%APPDATA%\SafeRoot\library\default_library.txt`
  - **影响文件**：`library_updater.py`（新增）、`main_window.py`

### 优化

- **crt.sh 查询稳定性增强**：超时时间从 20s 增加到 30s，新增最多 3 次自动重试机制（间隔 2s），
  并改进超时异常捕获，更精确识别网络超时场景。
